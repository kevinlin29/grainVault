...
const ImportRollModal = ({ onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(`Found ${images.length} images. Creating roll...`);
      
      // Generate a thumbnail from the first image
      const thumbnailPath = await window.electron.generateThumbnail(
      ...
      setError('Error importing roll: ' + error.message);
      setLoading(false);
    }
  };
